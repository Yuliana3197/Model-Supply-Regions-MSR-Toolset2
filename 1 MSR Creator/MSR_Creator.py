...
def RunResourceSufficiencyStage(Path_SuitableAreaResourceRaster,UserResourceThreshold,ResourceLowerLimit,RE_Technology,LandDiscount,
                                ...
                                DefaultMinContiguousAreaSuitableForMSR_kM2,Area_kM2):
    ...
    while (BreakWhileLoop == 0 and ResourceThreshold>=ResourceLowerLimit):
        ...
        SufficiencyCondition=Area_kM2*0.05
...
# Flag to define analysis mode Country or Region
...
#loop for country/region analysis
if AnalysisLevel == "country":
    iterator = [(ct.strip(), ct.strip()) for ct in AllCountries["Ct"]]
elif AnalysisLevel == "region":
    iterator = [(row["Country"].strip(), row["Region"].strip())for _, row in AllRegions.iterrows()]

for CountryName, RegionName_withSpaces in iterator:
    try:
        RegionName_withoutSpaces = RegionName_withSpaces.replace(" ", "")
        print(f"\n Process: {CountryName} - {RegionName_withSpaces}")
        ...
        if AnalysisLevel == "country":            
            ...
            RoadsBufferDistance_meters = 50 * 1000
            GridBufferDistance_meters = 0


    #assign and create folders
        OutputFolder = HomeDirectory +ControlPaths.loc["FolderAddress_OutputFolder"][0] + "/"
        if AnalysisLevel == "country":
            OutputFolder = OutputFolder+CountryName+ "/"

        elif AnalysisLevel == "region":
            OutputFolder = OutputFolder+f"region_{CountryName}/"+RegionName_withoutSpaces+ "/"
    ...
    #prepare country boundary shapefile
        if AnalysisLevel == "country":
            gdf_Selected = gdf_CountryBoundaries[
                gdf_CountryBoundaries.name == RegionName_withSpaces
                ]
        elif AnalysisLevel == "region":
            gdf_Selected = gdf_RegionBoundaries[
                (gdf_RegionBoundaries.name == RegionName_withSpaces) & (gdf_RegionBoundaries.geonunit == CountryName)
                ]

        if not os.path.isdir(SubfolderCountryMapsForClipping):
            os.makedirs(SubfolderCountryMapsForClipping)
        gdf_Selected.to_crs('EPSG:4326').to_file(SubfolderCountryMapsForClipping+'/' + RegionName_withoutSpaces + '.shp')
        Area_kM2 = gdf_Selected.to_crs("ESRI:54009").area.iloc[0] / 1000000

        for Script in Scripts:
            if Script == 1:
                UpperLeftX,LowerRightY, LowerRightX, UpperLeftY=gdf_Selected.total_bounds
                ...
                for Raster in [FileName_PopulationDensity, FileName_LandCover, FileName_Elevation, ResourceRasterName, FileName_ClimateZones]:
                    ...
                    ClippedRaster = ClippedRaster.rio.clip(gdf_Selected.geometry) #clip to country boundaries
                ...
                for Vector in [FileName_Roads, FileName_WaterBodies, FileName_PowerGrid, FileName_TransmissionGrid, FileName_DistributionGrid, FileName_ProtectedAreas]:
                    gdf_ClippedVector=gpd.read_file("%s%s.shp"%(InputSpatialDatasetsFolder,Vector), bbox=tuple(gdf_Selected.total_bounds)) #bbox (bounding box) used to avoid reading unwanted data. Note that this is does not clip features that extend beyond bbox boundary.
                    ...
                    if not gdf_ClippedVector.empty:
                        gdf_ClippedVector=gpd.clip(gdf_ClippedVector,gdf_Selected.envelope)
                        ...
                        gdf_ClippedVector = gpd.GeoDataFrame({'geometry': gdf_Selected.envelope}, geometry='geometry')
                        gdf_ClippedVector[
                    ...
                    gdf_ClippedVector = gpd.clip(gdf_ClippedVector, gdf_Selected.geometry)
                    if gdf_ClippedVector.empty: #Check again after clipping to borders that if the dataset is again empty then repeat the procedure of self loading appropriate polygon in order for next scoring step to function properly
                        gdf_ClippedVector = gpd.GeoDataFrame({'geometry': gdf_Selected.geometry},geometry='geometry')
                        gdf_ClippedVector["RasterValue"] = 1
                    ...
                    Rasterized_ClippedVector=Rasterized_ClippedVector.rio.clip(gdf_Selected.geometry)
                ...
                for DatasetName in [FileName_PowerGrid, FileName_TransmissionGrid, FileName_DistributionGrid, FileName_Roads]:
                    ...
                    DistanceSurface=DistanceSurface.rio.clip(gdf_Selected.envelope)
                    DistanceSurface = DistanceSurface.rio.clip(gdf_Selected.geometry)
            ...
            if Script == 2:
                ...
                SuitableAreaResourceRaster = SuitableAreaResourceRaster.rio.clip(gdf_Selected.geometry)
                ...
                CompetitiveAreaResourceRaster = CompetitiveAreaResourceRaster.rio.clip(gdf_Selected.geometry)
            ...
            if Script == 4:
                ...
                gdf_ClippedVector=gpd.read_file("%s%s.shp"%(InputSpatialDatasetsFolder,FileName_TransmissionGrid), bbox=tuple(gdf_Selected.total_bounds)) #bbox (bounding box) used to avoid reading unwanted data. Note that this is does not clip features that extend beyond bbox boundary.
                if not gdf_ClippedVector.empty:
                    gdf_ClippedVector = gpd.clip(gdf_ClippedVector, gdf_Selected.geometry)
                ...
                gpd_Substations = gpd.read_file("%s%s.shp"%(InputSpatialDatasetsFolder, FileName_Substations),bbox=tuple(gdf_Selected.total_bounds)).to_crs("ESRI:54009")
                ...
                gpd_LoadCenters = gpd.read_file("%s%s.shp"%(InputSpatialDatasetsFolder, FileName_UrbanAreaLoadCenters),bbox=tuple(gdf_Selected.total_bounds)).to_crs("ESRI:54009")
    ...
    # Extracts landuse, elevation statistics, and climate from raster datasets
        ...
        print(f"❌ Error en {CountryName} - {RegionName_withSpaces}: {e}")
        continue
