    ...
    for i in range (0,len(Countries)):
        ...
        try:
            if glob.glob("%s\\%s\\%s\\%s"%(Input_MSR_Folder, country, MSR_DataCarryingSubFolderName,SolarPVNameConvention) + MSR_ShapeFileNameSuffix): #single iteration loop
                ...
                gdf_SingleCountry['CF']=0.92*pd_SolarProfiles.iloc[:,-8784:].sum(axis=1)*100/8784 #ensure excel table is FID sorted, 4% outage and 4% inverter and wiring losses applied as per IRENA-LBNL report
                gdf_SingleCountry['Y_GWh']=gdf_SingleCountry['CapacityMW']*gdf_SingleCountry['CF']*8784/100000
                ...
                if Flag_PrepareMSR_CostCharachteristics:
                    ...
                    gdf_SingleCountry['sLCOE-MWh'] = 1000 * ((CostParameters.loc["SolarPV"][0] * CostParameters.loc["SolarPV"][3] + CostParameters.loc["SolarPV"][1]/1000) / (8784 * gdf_SingleCountry['CF'] / 100)) + CostParameters.loc["SolarPV"][2]
                    ...
                    gdf_SingleCountry['tLCOE-MWh'] = ((CostParameters.loc["SolarPV"][5]* CostParameters.loc["SolarPV"][8]+CostParameters.loc["SolarPV"][6]) * gdf_SingleCountry['T_Dist_gf']+CostParameters.loc["SolarPV"][7]*CostParameters.loc["SolarPV"][8]) / (8784 * gdf_SingleCountry['CF'] / 100)
                    gdf_SingleCountry['tCAPEX-kW'] = (CostParameters.loc["SolarPV"][5]*gdf_SingleCountry['T_Dist_gf'] +
                                                      CostParameters.loc["SolarPV"][7]) / 1000
                    #road costs are for 50MW plant i.e. 50*8784*CF MWh
                    gdf_SingleCountry['rLCOE-MWh'] = ((CostParameters.loc["SolarPV"][10]* CostParameters.loc["SolarPV"][12]+CostParameters.loc["SolarPV"][11])*gdf_SingleCountry['RoadDist']) / (8784 *50* gdf_SingleCountry['CF'] / 100)
    ...
    for i in range (0,len(Countries)):
        ...
        try:
            if glob.glob("%s\\%s\\%s\\%s"%(Input_MSR_Folder, country, MSR_DataCarryingSubFolderName, WindNameConvention) + MSR_ShapeFileNameSuffix): #single iteration loop
                ...
                gdf_SingleCountry[AnnualCFColName]=0.83*pd_WindProfiles.iloc[:,-8784:].sum(axis=1)*100/8784 #ensure excel table is FID sorted, 2% outage loss and 15% array and collection losses applied as per IRENA-LBNL report
                gdf_SingleCountry[AnnualYieldColName]=gdf_SingleCountry['CapacityMW']*gdf_SingleCountry[AnnualCFColName]*8784/100000
                ...
                if Flag_PrepareMSR_CostCharachteristics:
                    ...
                    for i in [0,1,2]:
                        #LCOEs calculated per kWH and scaled to MWh
                        #first element is in kWH so must be multiplied with 1000 to get MW basis. 2nd element is MW basis, so simply added.
                        gdf_SingleCountry.loc[gdf_SingleCountry['IEC_Class']==var1[i],'sLCOE-MWh'] = 1000 * ((CostParameters.loc[var2[i]][0] * CostParameters.loc[var2[i]][3] + CostParameters.loc[var2[i]][1]/1000) / (8784 * gdf_SingleCountry.loc[gdf_SingleCountry['IEC_Class']==var1[i],AnnualCFColName] / 100)) + CostParameters.loc[var2[i]][2]
                        #TL parameters are in MW basis so the LCOE inherently comes in MWh. Substation costs are for 2 substations suitable for an average 50MW POA/plant. So they must be converted to per MW basis.
                        gdf_SingleCountry.loc[gdf_SingleCountry['IEC_Class']==var1[i],'tLCOE-MWh'] = ((CostParameters.loc[var2[i]][5]* CostParameters.loc[var2[i]][8]+CostParameters.loc[var2[i]][6]) * gdf_SingleCountry.loc[gdf_SingleCountry['IEC_Class']==var1[i],'T_Dist_gf']+CostParameters.loc[var2[i]][7]*CostParameters.loc[var2[i]][8]) / (8784 * gdf_SingleCountry.loc[gdf_SingleCountry['IEC_Class']==var1[i],AnnualCFColName] / 100)
                        ...
                        #road costs are for 50MW plant i.e. 50*8784*CF MWh
                        gdf_SingleCountry.loc[gdf_SingleCountry['IEC_Class']==var1[i],'rLCOE-MWh'] = ((CostParameters.loc[var2[i]][10]* CostParameters.loc[var2[i]][12]+CostParameters.loc[var2[i]][11])*gdf_SingleCountry.loc[gdf_SingleCountry['IEC_Class']==var1[i],'RoadDist']) / (8784 *50* gdf_SingleCountry.loc[gdf_SingleCountry['IEC_Class']==var1[i],AnnualCFColName] / 100)
